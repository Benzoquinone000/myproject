...
该工具会查询上传的图谱，以及（如有）LightRAG生成的图谱。默认会查询所有LightRAG图谱；在智能体配置中如果选择了知识库，会自动限制查询范围。
"""


def _convert_graph_to_triples(nodes: list, edges: list) -> list:
    """将节点和边转换为三元组格式"""
    triples = []
    # 创建节点ID到名称的映射
...
def query_knowledge_graph(query: Annotated[str, "The keyword to query knowledge graph."]) -> Any:
    """不带知识库限制的默认查询版本（兼容旧行为）。"""
    return query_knowledge_graph_impl(query=query, db_names=None)


def query_knowledge_graph_impl(query: str, db_names: list[str] | None = None) -> Any:
    ...
    try:
        ...
        try:
            ...
            if isinstance(upload_result, dict) and "triples" in upload_result:
                all_triples.extend(upload_result["triples"])
        ...
        try:
            ...
            # 如果配置里选择了知识库，则只查询对应的 LightRAG 图谱
            if db_names:
...
def make_knowledge_graph_tool(db_names: list[str] | None = None) -> Any:
    """
    创建一个“查询知识图谱”工具实例，并将查询范围绑定到指定知识库（db_names）。
    注意：此工具的 name 仍然是“查询知识图谱”，以便前端/LLM 选择保持一致。
    """

    @tool(name_or_callable="查询知识图谱", description=KG_QUERY_DESCRIPTION)
    def _query_knowledge_graph_bound(
        query: Annotated[str, "The keyword to query knowledge graph."],
    ) -> Any:
        return query_knowledge_graph_impl(query=query, db_names=db_names)

    return _query_knowledge_graph_bound
