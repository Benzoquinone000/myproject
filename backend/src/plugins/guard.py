...
def _default_keywords_path() -> str:
    """返回 bad_keywords.txt 的路径，兼容本地与 Docker 环境"""
    return str(Path(__file__).resolve().parent.parent / "config" / "static" / "bad_keywords.txt")


class ContentGuard:
    def __init__(self, keywords_file: str | None = None):
        keywords_file = keywords_file or _default_keywords_path()
