...
class UploadGraphService:
            ...
            if entity_name:
                ...
                if os.path.exists(info_file_path):
                    try:
                        ...
                        logger.info("已删除图数据库配置文件，允许重新配置嵌入模型")
                    except Exception as e:
                        logger.warning(f"删除配置文件失败: {e}")
                # 保存更新后的配置（此时 is_initialized_from_file 为 False，不会锁定配置）
    ...
    def _delete_all_entities(self, tx):
        # 只删除带有 Entity 标签的节点，排除 LightRAG 的节点（带有 kb_ 开头标签的节点）
        # LightRAG 节点使用 kb_ 开头的标签来区分
        query = """
        MATCH (n:Entity)
        WHERE NOT ANY(label IN labels(n) WHERE label STARTS WITH 'kb_')
        DETACH DELETE n
        """
        tx.run(query)
    ...
    def save_graph_info(self, graph_name="neo4j"):
        ...
        try:
            ...
            # 只有在有实体数据时才标记为从文件初始化（锁定配置）
            # 如果实体数量为0，保持 is_initialized_from_file 为 False，允许重新配置
            if graph_info.get("entity_count", 0) > 0:
    ...
    def load_graph_info(self):
        ...
        try:
            ...
            # 只有在有实体数据时才标记为从文件初始化（锁定配置）
            # 如果实体数量为0，保持 is_initialized_from_file 为 False，允许重新配置
            entity_count = graph_info.get("entity_count", 0)
            if entity_count > 0:
                self.is_initialized_from_file = True
            else:
                self.is_initialized_from_file = False
