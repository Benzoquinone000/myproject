...
def _upload_image_to_minio_for_docling(image_data: bytes, filename: str, db_id: str) -> str:
    """上传 Docling 提取的图片到 MinIO，返回 URL"""
...
def _convert_with_docling(file_path: Path, params: dict | None = None) -> str:
    ...
    if hasattr(doc, "pictures") and doc.pictures:
        ...
        for filename, image_data in image_refs:
            try:
                url = _upload_image_to_minio_for_docling(image_data, filename, db_id)
...
async def process_file_to_markdown(file_path: str, params: dict | None = None) -> str:
    ...
    try:
        ...
        if file_ext == ".pdf":
            ...
            result = await asyncio.to_thread(_convert_with_docling, file_path_obj, params)
