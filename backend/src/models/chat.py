...
async def test_chat_model_status(provider: str, model_name: str) -> dict:
    ...
    使用轻量级探测：max_tokens=1 + 短超时 + 不重试，快速判断模型是否可达。
    """
    try:
        ...
        def _quick_probe():
            """轻量探测：创建短超时客户端，发 max_tokens=1 请求，不走 retry"""
            from openai import OpenAI as _OpenAI

            probe_client = _OpenAI(
        ...
        response = await asyncio.to_thread(_quick_probe)
...
async def test_all_chat_models_status() -> dict:
    ...
    for provider, provider_info in config.model_names.items():
        for model_name in provider_info.models:
            ...
            tasks[model_id] = test_chat_model_status(provider, model_name)
