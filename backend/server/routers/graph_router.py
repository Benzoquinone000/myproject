...
async def get_subgraph(
    ...
    max_nodes: int = Query(150, description="最大节点数", ge=1, le=1000),
...
async def get_neo4j_nodes(
    ...
    num: int = Query(150, description="节点数量", ge=1, le=1000),
    current_user: User = Depends(get_admin_user),
):
...
@graph.post("/neo4j/delete-entities")
async def delete_neo4j_entities(
    entity_name: str | None = Body(None, description="实体名称，为空则删除全部实体"),
    kgdb_name: str | None = Body("neo4j", description="知识图谱数据库名称"),
    current_user: User = Depends(get_admin_user),
):
    """删除Neo4j图谱中的实体"""
    try:
