...
onUnmounted(() => {
  ...
  // 页面切换时不 abort 流，让后端继续跑完，避免产生 "发生错误: interrupted"。
  // 只重置 UI 状态，不中断网络请求。
  const threadId = currentChatId.value;
  if (threadId) {
    const threadState = chatState.threadStates[threadId];
    if (threadState) {
      // 解除 abort controller 引用但不调用 abort()
...
const hasPendingToolCalls = (history = []) => {
  return history.some(msg =>
    msg?.type === 'ai' &&
    Array.isArray(msg.tool_calls) &&
    msg.tool_calls.some(tc => tc?.status === 'pending' && !tc?.tool_call_result)
  );
};

const fetchThreadMessagesWithRetry = async ({
  agentId,
  threadId,
  initialDelay = 300,
  retries = 2,
  ...
  await fetchThreadMessages({ agentId, threadId, delay: initialDelay });
  for (let i = 0; i < retries; i++) {
    const history = threadMessages.value[threadId] || [];
    if (!hasPendingToolCalls(history)) break;
    await new Promise(resolve => setTimeout(resolve, retryDelay));
    await fetchThreadMessages({ agentId, threadId });
  }
};
...
const sendMessage = async ({ agentId, threadId, text, signal = undefined, imageData = undefined }) => {
  ...
  const requestData = {
    ...
    config: {
      thread_id: threadId,
      // 注意：agentConfig 来自后端 context，会包含随机生成的 thread_id/user_id。
      // 发送聊天时必须以当前 threadId 为准，否则会把消息写到错误线程导致“对话消失”。
      ...(() => {
        const cfg = { ...(agentConfig.value || {}) };
        delete cfg.thread_id;
        delete cfg.user_id;
        return cfg;
      })(),
    },
