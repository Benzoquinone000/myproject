<template>
  <div class="agent-config-sidebar" :class="{ 'open': isOpen }">
    ...
    <div class="sidebar-content">
            ...
            <template v-for="(value, key) in configurableItems" :key="key">
              ...
              >
                ...
                <div v-else-if="key === 'system_prompt'" class="system-prompt-container">
                  ...
                  />
                  <div
                    v-if="systemPromptEditMode && value?.default !== undefined"
                    class="system-prompt-actions"
                  >
                    <a-button
                      type="link"
                      size="small"
                      class="reset-system-prompt-btn"
                      @click.stop="resetSystemPromptToDefault(value?.default)"
                    >
                      恢复默认
                    </a-button>
                  </div>
                  ...
                  <div class="options-grid">
                    ...
                    >
                      <div class="option-content">
                        <span class="option-text">{{ getOptionDisplayLabel(value, option) }}</span>
      ...
      <!-- 固定在侧栏底部（与中间滚动区分离，避免长表单裁切；高度随父级而非 100vh，防止嵌套布局里底部被 overflow 裁掉） -->
      <div class="sidebar-footer" v-if="selectedAgentId && selectedAgent">
...
// 打开侧栏时强制拉取最新 configurable_items（含 MCP 等动态选项），避免缓存缺项
watch(
  () => props.isOpen,
  async (open) => {
    if (!open || !selectedAgentId.value) return;
    try {
      // 并发加载，不串行等待
      await Promise.all([
        agentStore.fetchAgentDetail(selectedAgentId.value, true),
        agentStore.loadAgentConfig(selectedAgentId.value),
      ]);
    } catch (e) {
      console.warn('打开配置时刷新智能体详情失败:', e);
    }
  }
);
...
/** 多选项展示名（如 MCP 的 option_labels）；无映射时显示原值 */
const getOptionDisplayLabel = (value, option) => {
  const labels = value?.option_labels;
  if (labels && typeof labels === 'object' && labels[option] != null && labels[option] !== '') {
...
const openToolsModal = async () => {
  ...
  try {
    // 先打开弹窗，后台刷新工具列表，不阻塞用户交互
    selectedTools.value = [...(agentConfig.value?.tools || [])];
    toolsModalOpen.value = true;
    // 后台静默刷新最新工具列表
    if (selectedAgentId.value) {
      agentStore.fetchAgentDetail(selectedAgentId.value, true).catch(() => {});
    }
  } catch (error) {
...
// 系统提示词恢复默认
const resetSystemPromptToDefault = (defaultValue) => {
  // defaultValue 可能是字符串，也可能是 undefined
  agentStore.updateAgentConfig({
    system_prompt: defaultValue ?? ''
  });
...
.agent-config-sidebar {
  position: relative;
  width: 0;
  /* 横向 flex 子项默认 stretch，与聊天区同高；不写 100vh，避免高于父级时底部（含保存按钮）被 overflow 裁切 */
  min-height: 0;
  align-self: stretch;
      ...
      .config-form {
        ...
        .config-item {
          ...
          .system-prompt-input {
            ...
            padding: 8px 12px;
            max-height: 220px; // 防止系统提示词过长把底部保存按钮挤出可视区域
            overflow-y: auto;
  ...
  .reset-system-prompt-btn {
    ...
    line-height: 26px;
          ...
          .system-prompt-display {
            ...
            max-height: 220px; // 展示模式也限制高度，避免挤压页脚
            overflow-y: auto;
            ...
            .system-prompt-content {
               ...
               color: var(--gray-900);
               font-size: 14px;
               max-height: 190px; // 留出 edit-hint 与 padding 空间
               overflow-y: auto;
              //  min-height: 100px;
