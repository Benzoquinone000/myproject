...
import { extractChartImageUrl, isChartLikeToolName } from '@/utils/toolResultImages';
...
const isImageResult = computed(() => {
  if (!isChartLikeToolName(toolName.value)) return false;
  const url = extractChartImageUrl(props.toolCall.tool_call_result?.content);
  return !!url;
});
