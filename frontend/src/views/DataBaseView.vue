<template>
  <div class="database-container layout-container">
    <HeaderComponent title="知识库管理" :loading="dbState.listLoading">
      <template #actions>
        <a-button type="primary" @click="state.openNewDatabaseModel=true" style="border-radius: 8px;">
        ...
        <h3 style="margin-top: 20px;">
          <span style="display: inline-flex; align-items: center; gap: 6px;">
            ...
            <a-tooltip :title="selectedPresetDescription">
              <QuestionCircleOutlined style="color: var(--gray-500); cursor: help; font-size: 14px;" />
            </a-tooltip>
          </span>
        </h3>
        <a-select
...
const chunkPresetOptions = CHUNK_PRESET_OPTIONS.map(({ value, label }) => ({ value, label }))
...
// Restore info display via a pseudo layout
.dbcard .top + .description {
  // The actual info is shown elsewhere
}

// Override: show name/meta in a different way
.database-container .databases .database {
  .top {
    .info {
      display: flex;
      flex-direction: column;
