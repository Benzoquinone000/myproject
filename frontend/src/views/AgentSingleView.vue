    ...
    >
      <template #header-left>
        ...
        <div
          ...
          title="打开配置"
        >
          <Settings2 size="16" class="nav-btn-icon"/>
        </div>
      ...
      <template #header-right>
        <div type="button" class="agent-nav-btn compact" @click="handleShareChat">
...
import { Share2, ChevronDown, Settings2 } from 'lucide-vue-next';
...
onMounted(async () => {
  ...
  // 单智能体页面需要主动同步 store 的 selectedAgentId，
  // 否则配置侧边栏会显示错误的智能体配置。
  try {
    if (agentId.value) {
      await agentStore.selectAgent(agentId.value);
      if (agentStore) {
        await agentStore.loadAgentConfig(agentId.value);
      }
    }
  } catch (error) {
    console.error('同步单智能体配置失败:', error);
  }
});
...
watch(agentId, async (newId) => {
  if (!newId) return;
  try {
    await agentStore.selectAgent(newId);
    await agentStore.loadAgentConfig(newId);
  } catch (error) {
    console.error('路由切换后同步单智能体配置失败:', error);
  }
});
</script>
