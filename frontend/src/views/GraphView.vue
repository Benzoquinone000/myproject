    ...
    >
      <template #actions>
        ...
        <a-button
          ...
          :disabled="!graphInfo || (graphInfo.entity_count === 0 && graphInfo.relationship_count === 0)"
...
import { UploadOutlined, SyncOutlined, GlobalOutlined, InfoCircleOutlined, SearchOutlined, ReloadOutlined, LoadingOutlined, HighlightOutlined, DatabaseOutlined, ExportOutlined, DeleteOutlined } from '@ant-design/icons-vue';
...
const loadGraphInfo = () => {
  ...
  neo4jApi.getInfo()
    .then(data => {
      console.log('加载图谱信息:', data)
      graphInfo.value = data.data
      console.log('embed_model_configurable:', graphInfo.value?.embed_model_configurable)
      console.log('embedModelConfigurable computed:', embedModelConfigurable.value)
...
const showDeleteConfirm = () => {
  const nodeCount = graphInfo.value?.entity_count || 0;
  const edgeCount = graphInfo.value?.relationship_count || 0;
  
  Modal.confirm({
    title: '确认清空图谱',
    content: h('div', [
      h('p', { style: { marginBottom: '8px' } }, '此操作将删除图数据库中的所有实体和关系，且无法恢复。'),
      h('p', { style: { color: 'var(--color-warning)', fontWeight: 500 } }, 
        `当前图谱包含 ${nodeCount} 个节点和 ${edgeCount} 条边。`
      )
    ]),
    okText: '确认删除',
    cancelText: '取消',
    ...
    onOk: async () => {
      try {
        state.processing = true;
        const res = await neo4jApi.deleteEntities(null, 'neo4j');
        if (res.success) {
          message.success(res.message || '图谱已清空');
          // 清空当前显示的图谱
          graph.clearGraph();
          // 刷新图谱信息
          await loadGraphInfo();
        ...
        message.error(error.message || '删除图谱失败');
      } finally {
        state.processing = false;
      }
    }
