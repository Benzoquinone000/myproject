...
export const unifiedApi = {
  ...
  getSubgraph: async (params) => {
    const { db_id, node_label = "*", max_depth = 2, max_nodes = 150 } = params
...
export const neo4jApi = {
  ...
  /**
   * 删除Neo4j图谱中的实体
   * @param {string|null} entity_name - 实体名称，为空则删除全部实体
   * @param {string} kgdb_name - Neo4j数据库名称（默认为'neo4j'）
   * @returns {Promise} - 删除结果
   */
  deleteEntities: async (entity_name = null, kgdb_name = 'neo4j') => {
    return await apiPost('/api/graph/neo4j/delete-entities', {
      entity_name: entity_name,
      kgdb_name: kgdb_name
    }, {}, true)
  }
